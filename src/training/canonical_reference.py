...
DATA = DATA_ROOT / "ETT-small" / "ETTh1.csv"
...
def partition(x: torch.Tensor, origin: int, sentinel: float = 0.0):
    ...
    mask[:, origin:origin+CONTEXT] = True
    return out, mask
...
def load_data():
    ...
    return values, train, val, test, mean, std


@torch.inference_mode()
def evaluate(model, loader, device, sentinel=0.0):
    model.eval(); rows = []
    for origin in range(PATCH):
        se = ae = n = 0
        for x, y in loader:
            x, y = x.to(device), y.to(device)
            v, m = partition(x, origin, sentinel)
            e = model(v, m) - y
...
def run(seed, out):
    seed_all(seed); values, train, val, test, mean, std = load_data()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = ControlledTransformerV2().to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-4)
    tl = DataLoader(Windows(values, train), batch_size=32, shuffle=True, pin_memory=device.type == "cuda")
    vl = DataLoader(Windows(values, val), batch_size=32, shuffle=False, pin_memory=device.type == "cuda")
    test_loader = DataLoader(Windows(values, test), batch_size=32, shuffle=False, pin_memory=device.type == "cuda")
    out = Path(out); out.mkdir(parents=True, exist_ok=True)
    train_rows, val_rows, best_state, best_val, selected_epoch = [], [], None, float("inf"), 15
    ...
    summary = {"experiment_id":"CANONICAL_CONTROLLED_V2", "model":"Controlled-Transformer-V2", "seed":seed,
               "dataset":"ETTh1", "context":CONTEXT, "horizon":HORIZON, "patch":PATCH, "stride":PATCH,
               "n_train":len(train), "n_validation":len(val), "n_test":len(test), "epochs":15, "batch_size":32,
    ...
    config = ROOT / "configs" / "reference.json"
    prov = {"experiment_id":"CANONICAL_CONTROLLED_V2", "seed":seed, "python":sys.version,
            "pytorch":torch.__version__, "cuda":torch.version.cuda,
            "gpu":torch.cuda.get_device_name(0) if torch.cuda.is_available() else "NONE",
            "config_sha256":sha256(config), "code_sha256":sha256(ROOT / "canonical_run.py"),
            "checkpoint_sha256":sha256(out / "checkpoint.pt"), "final_checkpoint_sha256":sha256(out / "final_checkpoint.pt")}
...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(); p.add_argument("--seed", type=int, required=True); p.add_argument("--out", required=True)
    a = p.parse_args(); run(a.seed, a.out)
