...
DATASET_PATHS = {
    ...
    "ETTh2": SOURCE_ROOT / "dataset" / "ETT-small" / "ETTh2.csv",
    "ETTm1": SOURCE_ROOT / "dataset" / "ETT-small" / "ETTm1.csv",
    "ETTm2": SOURCE_ROOT / "dataset" / "ETT-small" / "ETTm2.csv",
    "Weather": SOURCE_ROOT / "dataset" / "weather" / "weather.csv",
}
...
def train_controlled(seed: int, out: Path, strategy: str, context: int, horizon: int,
                     ...
                     dataset: str = "ETTh1") -> dict:
    seed_all(seed)
    data = load_data(dataset, horizon)
    dev = device()
    model = ControlledTransformerSupplement(context, horizon, patch_len, stride, data.channels).to(dev)
    ...
    config = {"experiment_id": "SUPPLEMENT_TRAIN_ORIGIN_STRATEGY" if strategy else "SUPPLEMENT_CONTROLLED",
...
def train_patchtst(seed: int, out: Path, context: int = L512, horizon: int = H96,
                   ...
                   epochs: int = 10, dataset: str = "ETTh1") -> dict:
    seed_all(seed)
    ...
    test_loader = DataLoader(Windows(data.raw, data.test, context, horizon),
                             ...
                             pin_memory=dev.type == "cuda")
    phases = list(range(phase_count(patch_len, stride)))
    config = {"experiment_id": "SUPPLEMENT_PATCHTST_ORIGIN",
...
def run_one(experiment: str, seed: int, dataset: str = "ETTh1", setting: str = "all") -> dict:
    ...
    if experiment == "2":
        ...
        for strategy in ("boundary_only", "random_origin", "all_origins"):
            results[strategy] = train_controlled(seed, base / "train_origin_strategy" / strategy / f"seed{seed}", strategy, L512, H96, PATCH_LEN, PATCH_LEN)
    ...
    if experiment == "3":
        ...
        for stride in strides:
            ...
            results[stride] = train_controlled(seed, out, "random_origin", L512, H96, PATCH_LEN, stride, dataset=dataset)
    ...
    if experiment == "4":
        ...
        return train_controlled(seed, out, "random_origin", L512, 192, PATCH_LEN, PATCH_LEN, dataset=dataset)
