...
def main():
    log=STAGE/"logs"/"batch1a_all.log"; log.parent.mkdir(parents=True,exist_ok=True)
    with log.open('w',encoding='utf8') as lf:
        for d,s in JOBS:
            out=STAGE/"batch1_inference_and_fixed_origin"/"inference_baselines"/d/f"seed{s}.json"
            if out.exists():
                lf.write(json.dumps({'status':'already_present','dataset':d,'seed':s})+'\n'); lf.flush(); continue
            cmd=[sys.executable,str(RUNNER),'--mode','baseline','--dataset',d,'--seed',str(s),'--out',str(out)]
            lf.write(json.dumps({'status':'start','dataset':d,'seed':s,'cmd':cmd})+'\n'); lf.flush()
