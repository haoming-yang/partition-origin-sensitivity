...
FROZEN = STAGE / "batch2_poc" / "FROZEN_POC_LAMBDA.json"
PROTOCOL = STAGE / "batch2_poc" / "lambda_selection" / "LAMBDA_SELECTION_PROTOCOL.json"
...
def reuse_seed42(lam: float, schedule_path: Path, schedule_hash: str, out: Path):
    src=STAGE/"batch2_poc"/"lambda_selection"/"lambda_1p0"
    if not (src/"summary.json").exists() or not (src/"checkpoint.pt").exists(): raise FileNotFoundError(src)
    ...
    dump(out/"FORMAL_REUSE.json",{"formal_reuse":True,"source_directory":str(src),"source_summary_sha256":sha(src/"summary.json"),"source_checkpoint_sha256":sha(src/"checkpoint.pt"),"test_evaluation_before_reuse":False,"initial_parameter_hash_C":initial_hash,"origin_pair_schedule_sha256":schedule_hash,"lambda":lam})
