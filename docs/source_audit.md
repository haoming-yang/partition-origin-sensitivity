...
The canonical measurement, overlap/H=192 extensions, origin-training strategies, isolated PatchTST adapter, patch-length runner, and full-split Transformer/MLP/Conv comparison are source-backed. The full-split runner was recovered from the historical workspace and copied under `tools/fullsplit/`. Its public copy adapts the data-root lookup to `PARTITION_ORIGIN_DATA_ROOT` or the repository `data/` directory, adds a package-import fallback for repository execution, records repository-relative source metadata, and accepts arbitrary integer seeds; the training and evaluation implementation is otherwise preserved. The optimization trajectory, mask-by-head factorial, and no-PE control remain runnable reconstructed controls. The POC source and schedules are included under `artifacts/poc_stage3/` and `artifacts/poc_stage4/`; the eight binary checkpoints remain external and are verified by the repository SHA256 manifest.
